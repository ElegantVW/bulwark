...
pub fn run() -> Result<()> {
    ...
    // Full scans (ward hunt, purity re-hash, /proc walks) are expensive —
    // run them at most every REFRESH seconds, not on every keystroke.
    ...
    loop {
        let frame = match screen {
            ...
            Screen::Purity => draw_purity(&mut cache, sel, &flash),
            Screen::Ward => draw_ward(&mut cache, list_sel, list_scroll, &flash),
            Screen::Sentinel => draw_sentinel(&mut cache, list_sel, list_scroll),
            ...
            Screen::Purity => match key {
                ...
                Key::Char('1') | Key::Char('p') => {
                    ...
                    flash = match take_photo() {
                        Ok(n) => {
                            cache.invalidate_purity();
                            format!("Purity photo saved ({n} files).")
                        }
            ...
            Screen::Ward => {
                ...
                match key {
                    ...
                    Key::Char('1') | Key::Char('w') | Key::Enter => {
                        ...
                        list_sel = 0;
                        list_scroll = 0;
                        cache.invalidate_ward();
                    }
                    _ => {}
                }
            }
            Screen::Sentinel => {
                let n = cache.sentinel().len().max(1);
                match key {
                    Key::Char('q') | Key::Ctrl('c') => break,
                    Key::Esc | Key::Char('b') | Key::Left => {
                        screen = Screen::Home;
...
fn keep_aegis() {
    // Write the confirm marker. The deadman watcher sees it and keeps the
    // rules; it never deletes the marker (a later apply/undo cleans it up).
...
fn draw_purity(cache: &mut ScanCache, sel: usize, flash: &str) -> String {
    let mut lines = vec![
    ...
    let (count, f) = cache.purity();
    if let Some(n) = count {
        lines.push(format!("Last photo: {n} files."));
        if f.is_empty() {
...
fn draw_ward(cache: &mut ScanCache, list_sel: usize, scroll: usize, flash: &str) -> String {
    let findings = cache.ward();
...
fn draw_sentinel(cache: &mut ScanCache, list_sel: usize, scroll: usize) -> String {
...
/// TTL cache for the expensive full-system scans. Every screen redraw used
/// to re-run the ward hunt (/proc environ walk), a full purity re-hash, and
/// a /proc fd walk — per keystroke. Now: at most once per REFRESH.
struct ScanCache {
    refresh: Duration,
    posture: Option<(std::time::Instant, Posture)>,
    ward: Option<(std::time::Instant, Vec<ward::Finding>)>,
    sentinel: Option<(std::time::Instant, Vec<String>)>,
    purity: Option<(
        std::time::Instant,
...
impl ScanCache {
    ...
    fn fresh(&self, t: &std::time::Instant) -> bool {
        t.elapsed() < self.refresh
    }

    fn posture(&mut self) -> Posture {
        let fresh = self.posture.as_ref().map(|(t, _)| self.fresh(t)).unwrap_or(false);
        if fresh {
            self.posture.as_ref().unwrap().1.clone()
        } else {
            let p = words::gather_posture();
            self.posture = Some((std::time::Instant::now(), p.clone()));
            p
        }
    }

    fn ward(&mut self) -> Vec<ward::Finding> {
        let fresh = self.ward.as_ref().map(|(t, _)| self.fresh(t)).unwrap_or(false);
        if fresh {
            self.ward.as_ref().unwrap().1.clone()
        } else {
            let f = ward::hunt();
            self.ward = Some((std::time::Instant::now(), f.clone()));
            f
        }
    }

    fn invalidate_ward(&mut self) {
        self.ward = None;
    }

    fn invalidate_purity(&mut self) {
        self.purity = None;
    }

    fn sentinel(&mut self) -> Vec<String> {
        let fresh = self
            .sentinel
            .as_ref()
            .map(|(t, _)| self.fresh(t))
            .unwrap_or(false);
        if fresh {
            self.sentinel.as_ref().unwrap().1.clone()
        } else {
            let s = words::sentinel_plain_lines();
            self.sentinel = Some((std::time::Instant::now(), s.clone()));
            s
        }
    }

    /// (file count, findings). None count = no baseline. Invalidated when
    /// the baseline file's mtime changes or the TTL expires.
    fn purity(&mut self) -> (Option<usize>, Vec<purity::Finding>) {
        let path = paths::purity_baseline_path();
        let mtime = fs::metadata(&path).and_then(|m| m.modified()).ok();
        let fresh = self
            .purity
            .as_ref()
            .map(|(t, mt, _, _)| self.fresh(t) && *mt == mtime)
            .unwrap_or(false);
        if fresh {
            let (_, _, n, f) = self.purity.as_ref().unwrap();
            (*n, f.clone())
        } else {
            let (n, f) = match purity::load_baseline(&path) {
                Ok(bl) => (Some(bl.files.len()), purity::check(&bl)),
                Err(_) => (None, Vec::new()),
            };
            self.purity = Some((std::time::Instant::now(), mtime, n, f.clone()));
            (n, f)
        }
    }
}
...
impl Term {
    ...
    pub fn read_key(&mut self) -> Result<Key> {
        ...
        // EOF on stdin (closed tty/pipe) behaves like quit — never spin.
        let b0 = match read_byte_raw() {
            None => return Ok(Key::Ctrl('c')),
            Some(b) => b,
        };
        Ok(match b0 {
            b'\n' | b'\r' => Key::Enter,
