...
pub const NFT_PAYLOAD_NETWORK_HEADER: u32 = 0;
...
impl Netlink {
    ...
    /// Send a whole nf_tables batch (begin … end) in one skb and wait for the
    /// kernel verdict (single ACK on the end message; per-message ACKs are
    /// tolerated). Any NACK aborts the batch atomically in the kernel.
    pub fn send_batch(&mut self, msgs: &[Vec<u8>]) -> Result<()> {
        let fd = self.fd.as_raw_fd();
        let mut buf = Vec::new();
        for m in msgs {
            buf.extend_from_slice(m);
        }
        // NOTE: the BATCH_BEGIN message keeps its own short nlmsg_len —
        // nfnetlink_rcv_skb_batch() pulls the begin by its own length and
        // walks the rest from skb->len.
        let n = unsafe { libc::send(fd, buf.as_ptr() as *const c_void, buf.len(), 0) };
        if n < 0 {
            return Err(io::Error::last_os_error()).context("netlink batch send");
        }
        // The kernel delivers one verdict per ACK-flagged message, plus an
        // NLMSG_ERROR for every failing message (even without NLM_F_ACK).
        // Errors can follow earlier success acks, so we must scan the whole
        // reply stream for any non-zero error before declaring success.
        ...
        loop {
            ...
            if pr < 0 {
                return Err(io::Error::last_os_error()).context("netlink batch poll");
            }
            if pr == 0 {
                // no more replies: success iff we saw a clean ack
                if saw_ack {
                    return Ok(());
                }
                if std::time::Instant::now() > deadline {
                    bail!("netlink batch: no kernel verdict (timeout)");
                }
                continue;
            }
            let n = unsafe { libc::recv(fd, rbuf.as_mut_ptr() as *mut c_void, rbuf.len(), 0) };
            if n < 0 {
                return Err(io::Error::last_os_error()).context("netlink batch recv");
            }
            if std::env::var("BULWARK_NL_DEBUG").is_ok() {
                eprintln!("netlink reply hex: {}", hex::encode(&rbuf[..n as usize]));
                let mut off = 0usize;
                let mut msgs = Vec::new();
                while off + mem::size_of::<Nlmsghdr>() <= n as usize {
                    let hdr = unsafe { &*(rbuf.as_ptr().add(off) as *const Nlmsghdr) };
                    let mut tag = format!("type={:#06x} len={}", hdr.nlmsg_type, hdr.nlmsg_len);
                    if hdr.nlmsg_type == NLMSG_ERROR {
                        let err_off = off + mem::size_of::<Nlmsghdr>();
                        if err_off + 4 <= n as usize {
                            let mut ebytes = [0u8; 4];
                            ebytes.copy_from_slice(&rbuf[err_off..err_off + 4]);
                            tag.push_str(&format!(" err={}", i32::from_ne_bytes(ebytes)));
                        }
                    }
                    msgs.push(tag);
                    let len = hdr.nlmsg_len as usize;
                    if len < mem::size_of::<Nlmsghdr>() || off + len > n as usize {
                        break;
                    }
                    off += align4(len);
                }
                eprintln!("netlink reply messages: {}", msgs.join(" | "));
            }
...
/// First NLMSG_ERROR code in a reply buffer, if any (skips object messages).
/// Scan a whole netlink reply buffer for NLMSG_ERROR verdicts.
/// Returns (any ack seen, first non-zero error). Errors are always delivered
/// by the kernel even without NLM_F_ACK, and may follow earlier success acks.
fn scan_replies(buf: &[u8]) -> Result<(bool, Option<i32>)> {
    let mut saw_ack = false;
    let mut err: Option<i32> = None;
    if buf.len() < mem::size_of::<Nlmsghdr>() {
        return Ok((saw_ack, err));
    }
    let mut off = 0;
    while off + mem::size_of::<Nlmsghdr>() <= buf.len() {
        ...
        if hdr.nlmsg_type == NLMSG_ERROR {
            ...
            if err_off + 4 <= buf.len() {
                ...
                let e = i32::from_ne_bytes(ebytes);
                if e != 0 && err.is_none() {
                    err = Some(e);
                }
            }
...
fn first_error(buf: &[u8]) -> Result<Option<i32>> {
    if buf.len() < mem::size_of::<Nlmsghdr>() {
        return Ok(None);
    }
    let mut off = 0;
    while off + mem::size_of::<Nlmsghdr>() <= buf.len() {
        ...
        if hdr.nlmsg_type == NLMSG_ERROR {
            ...
            if err_off + 4 <= buf.len() {
                let mut ebytes = [0u8; 4];
                ebytes.copy_from_slice(&buf[err_off..err_off + 4]);
                return Ok(Some(i32::from_ne_bytes(ebytes)));
            }
            return Ok(Some(0));
        }
        off += align4(len);
...
/// Low-level message builder. `flags` are used verbatim (no auto-ACK);
/// `res_id` is the nfgenmsg res_id (network byte order expected by caller).
fn nlmsg_raw(seq: u32, msg_type: u16, flags: u16, family: u8, res_id: u16, payload: &[u8]) -> Vec<u8> {
...
pub fn nlmsg(seq: u32, msg_type: u16, flags: u16, family: u8, payload: &[u8]) -> Vec<u8> {
    nlmsg_raw(seq, msg_type, flags | NLM_F_ACK, family, 0, payload)
}

/// nf_tables batch begin. res_id carries the subsystem (nftables).
...
/// nf_tables batch end. NLM_F_ACK on the end message asks the kernel for
/// the single batch verdict (matches libmnl/mnl_nlmsg_batch_end).
...
/// Build GETTABLE message — used to probe live kernel state.
pub fn msg_get_table(seq: u32, family: u8, name: &str) -> Vec<u8> {
    let mut payload = Vec::new();
    payload.extend(nla_put_str(NFTA_TABLE_NAME, name));
    nlmsg(
        seq,
...
pub fn expr_cmp_eq_u32_be(reg: u32, val: u32) -> Vec<u8> {
    let mut data = Vec::new();
    data.extend(nla_put_u32(NFTA_CMP_SREG, reg));
    data.extend(nla_put_u32(NFTA_CMP_OP, NFT_CMP_EQ));
    let be = val.to_be_bytes();
...
pub fn expr_cmp_eq_bytes(reg: u32, bytes: &[u8]) -> Vec<u8> {
    let mut data = Vec::new();
    data.extend(nla_put_u32(NFTA_CMP_SREG, reg));
    data.extend(nla_put_u32(NFTA_CMP_OP, NFT_CMP_EQ));
    let mut d = Vec::new();
    d.extend(nla_put(NFTA_DATA_VALUE, bytes));
    data.extend(nla_nested(NFTA_CMP_DATA, &d));
    expr("cmp", &data)
}
...
/// Network-layer payload (e.g. IPv4 saddr at offset 12 len 4,
/// IPv6 saddr at offset 8 len 16). Bytes come out in wire (BE) order.
pub fn expr_payload_network(reg: u32, offset: u32, len: u32) -> Vec<u8> {
    let mut data = Vec::new();
    data.extend(nla_put_u32(NFTA_PAYLOAD_DREG, reg));
    data.extend(nla_put_u32(NFTA_PAYLOAD_BASE, NFT_PAYLOAD_NETWORK_HEADER));
    data.extend(nla_put_u32(NFTA_PAYLOAD_OFFSET, offset));
    data.extend(nla_put_u32(NFTA_PAYLOAD_LEN, len));
    expr("payload", &data)
}

/// meta nfproto load (compare with a following cmp expr; the packet family
/// for inet tables is NFPROTO_IPV4 / NFPROTO_IPV6).
pub fn expr_meta_nfproto(reg: u32) -> Vec<u8> {
    let mut data = Vec::new();
    data.extend(nla_put_u32(NFTA_META_DREG, reg));
    data.extend(nla_put_u32(NFTA_META_KEY, NFT_META_NFPROTO));
    expr("meta", &data)
}
...
pub fn expr_cmp_neq_u32(reg: u32, val: u32) -> Vec<u8> {
    let mut data = Vec::new();
    data.extend(nla_put_u32(NFTA_CMP_SREG, reg));
    data.extend(nla_put_u32(NFTA_CMP_OP, NFT_CMP_NEQ));
    let mut d = Vec::new();
