...
const PAGES: &[&[&str]] = &[
    ...
    &[
        "Grown-up password",
        "",
        "Raise Aegis may ask for your password.",
        "That is sudo — Bulwark never stores it.",
        "Then run: bulwark aegis confirm  (no password).",
        "Keep after reboot: bulwark install --system",
    ],
    &[
        "Sentinel — strangers vs only-here",
        "",
        "Open windows can face this computer only,",
        "or the whole network.",
        "A magic fae door on the whole network is DANGER.",
    ],
    &[
        "Your first quest",
        "",
        "1) Purity photo (menu 1)",
        "2) Raise Aegis (menu 4) + confirm",
        "3) install --system if you want the wall after reboot",
        "",
